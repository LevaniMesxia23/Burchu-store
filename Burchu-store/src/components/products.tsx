...
function Products() {
  return (
    <div className=' text-white grid gap-8 p-8 min-h-screen grid-cols-2 sm:grid-cols-2 lg:grid-cols-4'>
      {data.product.map((item, index) => (
        <div key={index} className='flex flex-col items-start rounded-lg p-4'>
          ...
          <p className='mb-2 text-center text-[2rem] text-black'>{item.name}</p>
